...
_DEFAULT_CONFIG = {
    ...
    'weights': {'ml': 0.65, 'dynamic': 0.25, 'virustotal': 0.1},
    'thresholds': {'malicious': 0.75},
    'sandbox': {'timeout': 8},
...
def calculate_final_score(ml_result, dynamic_result, vt_result):
    try:
        weights = config.get('weights', {'ml': 0.65, 'dynamic': 0.25, 'virustotal': 0.1})
...
# Redirect detection
    if 'redirect' in url_lower or 'url=' in url_lower:
     dynamic_result['redirects'] = 1
     dynamic_result['events']['redirects'].append(url)
     dynamic_result['suspicious_indicators'] += 1
     dynamic_result['dynamic_score'] += 0.2

# Suspicious JavaScript
    if '.js' in url_lower or 'script' in url_lower:
     dynamic_result['suspicious_js'] = 1
     dynamic_result['events']['suspicious_js'].append(url)
     dynamic_result['suspicious_indicators'] += 1
     dynamic_result['dynamic_score'] += 0.2
...
# Forms / phishing indicators
    ...
    'account'
     ...
     dynamic_result['forms'] = 1
     dynamic_result['events']['form_submissions'].append(url)
     dynamic_result['suspicious_indicators'] += 1
     dynamic_result['dynamic_score'] += 0.2
...
# Downloads
    ...
    '.rar'
     ...
     dynamic_result['downloads'] = 1
     dynamic_result['events']['downloads'].append(url)
     dynamic_result['suspicious_indicators'] += 1
     dynamic_result['dynamic_score'] += 0.2

# Cookies / sessions
    if 'session' in url_lower or 'token' in url_lower:
     dynamic_result['cookies'] = 1
     dynamic_result['events']['cookies'].append(url)
     dynamic_result['suspicious_indicators'] += 1
     dynamic_result['dynamic_score'] += 0.2
    ...
    try:
       ...
       # if not guest_mode:
        ...
        result = {
            ...
            'ml_result': ml_result if isinstance(ml_result, dict) else {'label': 'error', 'confidence': 0.0, 'probabilities': {'safe': 0.0,'malicious': 0.0}},
        ...
        try:
            ...
            record_analysis(url, result, user_id)
        except Exception as e:
...
def get_stats_api():
    try:
        user = get_user_from_session()
        user_id = user['id'] if user else 'anonymous'
        print("STATS USER ID = ", user_id)
        stats = get_stats(user_id)
        if not isinstance(stats, dict):
            stats = {
                'urls': {'total': 0, 'malicious': 0,  'safe': 0}, 
...
def save_result():
    try:
        ...
        created_at=saved_result['saved_at']
        ) 
